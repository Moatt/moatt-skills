...
description: >
  Pull visual branding (colors, typography, layout patterns) from a client's website
  and emit a style preset compatible with the HTML slides skill plus a brand config
  JSON for the content asset creator. WebFetch reads the pages; the agent then
  analyzes the CSS/HTML to identify the palette, font pairings, and aesthetic patterns.
tags: [brand]
---

# Visual Brand Extractor

Lift a client's visual identity off their website and generate reusable style presets for slides and content assets. This is an agent-executed skill — the AI reads pages via WebFetch and runs the analysis directly.
...
|-------|----------|-------------|
| **Website URL** | Yes | Client's homepage or a landing page URL |
| **Client name** | Yes | Used to name the output files |
...
2. **Brand config JSON** — Lightweight color/font config matching the `skills/content-asset-creator` format
...
Pull 2-3 pages to get a representative slice of the brand:
...
### Phase 2: Extract the Color Palette

Analyze the fetched content. Hunt for the palette in this priority order:

#### 2.1 CSS Custom Properties

Look at `:root`, `html`, or `body` blocks for color variables:
```css
...
Check for:
- `<meta name="theme-color" content="#...">` — usually the primary brand color
...
|------|----------------|
| **bg-primary** | `background-color` on `body`, `html`, or the outermost container. The most common background. |
...
| **accent** | The most prominent non-bg, non-text color. Found on buttons, links, CTAs, highlights. Also check `<meta name="theme-color">`. |
| **accent-secondary** | A second accent color, or gradient partner. Found on secondary buttons, hover states. |
| **card-bg** | Background of cards, modals, or elevated surfaces (if different from bg-primary). |

#### 2.6 Determine the Theme Type
...
- **Mixed:** Note when the site uses both (e.g., dark hero + light content sections)
...
Hunt for font loading in this order:
...
2. **Fontshare `<link>` tags** — Similar pattern:
   ```
...
If both use the same family, it's a single-family pairing (like "Manrope + Manrope"). Note the different weights — display (700-900) vs body (400-500).
...
Inspect the CSS and page structure for these aesthetic signals:
...
From the signals above, compose:
...
Create `clients/<client-name>/brand/visual-identity.md` containing both output formats.
...
**Layout:** [1 sentence describing the dominant layout pattern]
...
When a site uses a proprietary font not on Google Fonts or Fontshare, use this mapping to pick the closest available alternative:
...
**Decision heuristic:** If you're unsure which category a font lands in, look at its features:
- Round dots and a geometric 'o' → Geometric sans
- Variable stroke widths, warm feel → Humanist sans
...
**Note:** Tailwind classes appear as `bg-blue-600`, `text-gray-900`, `border-indigo-500`, etc. Strip the property prefix (`bg-`, `text-`, `border-`) to get the color token, then look up the hex above.
...
Modern sites (Next.js, React) often inject styles via JavaScript. WebFetch may return minimal inline CSS. In that case:
- Focus on Tailwind utility classes in `class` attributes
- Check `<script>` tags for embedded config objects
- Look for `<style>` tags carrying CSS-in-JS output
- If it's still sparse, ask the user: "The site appears to use dynamic styling. Could you share a screenshot or provide their brand guidelines directly?"
...
### Too Many Colors
Enterprise sites may run dozens of colors. Focus on:
- The hero/above-the-fold section for the primary palette
- Button/CTA colors for accent identification
- Frequency analysis — the top 5-7 unique colors define the brand

### No Usable Font Information
If font detection comes up empty:
1. Check whether the site references font names in CSS comments or config
...
**Signature Elements:**
- Subtle gradient orbs in the background (green-to-blue radial gradients)
- Rounded cards with slate-800 background and a subtle border
...
*(This is illustrative — actual extraction results will vary based on the live site.)*

---

## Tips

- **The homepage is usually enough.** When it has clear branding, you don't need more pages. Pull a second page only if the homepage is sparse or runs a different aesthetic from the rest of the site.
- **Buttons reveal the accent color.** The most reliable way to identify the primary accent is finding the button/CTA background color.
- **Check the favicon/logo.** SVG logos often carry the exact brand colors as `fill` values.
- **Don't over-extract.** The target is 5-7 colors, 1-2 fonts, and 3-4 signature elements. More than that creates noise, not signal.
- **When in doubt, simplify.** A 3-color palette (bg + text + accent) with one font family at two weights makes a clean, usable preset.

## Dependencies

- Web fetch capability (to read website pages)

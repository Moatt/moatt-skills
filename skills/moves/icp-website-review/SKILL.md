...
description: >
  Review a website, landing page, content piece, or any online asset through the eyes
  of pre-built synthetic ICP personas. Loads personas from buyer-persona-generator output,
  then runs them against the target URLs. Ships in three modes: structured scorecard,
  freeform focus group, and head-to-head competitive comparison. Reusable — run it
  again against the same site after a change, or against new content any time.
tags: [research]
---

# ICP Website Review

Evaluate any online asset through the lens of pre-built synthetic ICP personas. This is the evaluation engine — it loads personas built by `buyer-persona-generator` and turns them loose on whatever you point them at.

## Prerequisites

Personas need to exist at `clients/<client>/personas/personas.json`. If they don't, run `buyer-persona-generator` first.
...
| **Specific pages** | No | Default: homepage + key pages surfaced during crawl |
| **Scope** | No | `full-site` (default) or a specific page URL for a single-page review |

## Modes

### Mode 1: Structured Scorecard (Default)

Rigorous, dimension-by-dimension evaluation backed by numerical scores. Strongest for:
- Pre-redesign audits
- Conversion optimization
- Tracking improvement over time (scores stay comparable across runs)
- Stakeholder presentations (data-driven)

### Mode 2: Freeform Focus Group

Each persona reacts naturally — stream of consciousness, emotional reactions, lingering questions, things that confuse them. No rigid scoring. Strongest for:
- Early-stage feedback on a draft or concept
- Surfacing emotional reactions and gut feelings
- Picking up unexpected issues the scorecard dimensions might miss
- More natural, human-sounding feedback

### Mode 3: Head-to-Head Competitive

Each persona reviews your site AND a competitor's, then picks a winner with reasoning. Strongest for:
- Competitive positioning
- Understanding why prospects pick competitors
- Pinpointing the specific places competitors out-execute you
...
Read `clients/<client>/personas/personas.json`. Confirm that:
- Personas exist and are well-formed
- You can list which ones will participate (name, title, segment)
- If the user wants only specific personas, filter accordingly

### Step 2: Crawl Target Pages

Pull the pages each persona would realistically visit:

1. **Homepage** (everyone lands here)
2. **Pricing page** (if it's public)
3. **Product/features page** (for technical buyers)
4. **Solutions/use-cases page** (for business buyers)
5. **About page** (trust/credibility signal)
6. **Case studies or testimonials** (social proof)
7. **Blog** (1-2 recent posts as a content-quality signal)
8. **Documentation or resources** (if relevant for technical buyers)

Use `WebFetch` on each page. Pull main content, headlines, CTAs, and the overall structure. Note what's present and what's missing.

Also check external presence:
- **Search results**: WebSearch "[company name]" and "[company name] reviews"
- **Review sites**: a quick G2/Capterra pass
- **Social proof signals**: notable logos, press coverage, trust badges

For **head-to-head mode**, crawl the same set of pages on the competitor site.
...
For each persona, evaluate these dimensions:

**A) First Impression (10 seconds)**
- What do they think this company does?
- Is it obvious this is for them?
- Does the headline speak to their pain?
- How much do they trust it at first glance?
- **Rating: 1-10**

**B) Messaging Relevance**
- Does the value proposition land on their specific pain?
...
- Close with a gut-level verdict: interested, skeptical, confused, or gone

After every persona has reacted, synthesize the common themes and the divergent opinions.

---

#### Head-to-Head Mode

For each persona, evaluate BOTH sites, then compare them.
...
Once every persona is done, produce a cross-persona competitive summary:
- Where does each site win across most personas?
- Which persona segments lean toward the competitor? (Those are at-risk segments)
- Which competitor advantages need neutralizing?
- Which of your unique advantages should you amplify?
...
This naming lets you accumulate reviews over time — track improvements, compare before/after redesigns, or stockpile competitive intel.
...
### At-risk segments
[Personas leaning toward the competitor — these ICPs may be churning or not converting]
...
Because personas are persisted separately, you can re-run this skill anytime:

```
Re-run the ICP website review for [client]. Use the existing personas.
```

Compare across dates to track improvement:
```
Compare today's ICP review for [client] against the one from [date].
```

The dated output path (`icp-reviews/<date>-<mode>.md`) makes it easy to flip through review history and see how scores move.
...
- **External presence matters.** What shows up when a persona Googles the company? Reviews? Press? That shapes trust before they ever hit the site.
- **Focus group mode is underrated.** Freeform reactions often surface things the scorecard misses — emotional reactions, unexpected confusion, things people wouldn't flag on a survey.
- **Head-to-head is the killer use case.** Nothing clarifies your positioning gaps faster than seeing your site through a buyer's eyes alongside the competitor's.
- **No code script lives here.** It's agent-executed using WebSearch and WebFetch.

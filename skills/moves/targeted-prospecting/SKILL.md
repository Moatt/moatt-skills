...
```bash
curl -s -X POST $MOATT_API_BASE/v1/proxy/apollo/mixed_people/search \
  -H "Authorization: Bearer $MOATT_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s -X POST $MOATT_API_BASE/v1/proxy/fiber/v1/natural-language-search/profiles \
  -H "Authorization: Bearer $MOATT_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s -X POST $MOATT_API_BASE/v1/proxy/pdl/person/search \
  -H "Authorization: Bearer $MOATT_API_KEY" \
  -H "Content-Type: application/json" \

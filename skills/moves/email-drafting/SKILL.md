...
name: email-drafting
description: >
  Compose high-converting cold emails with structured frameworks, personalization
  tiers, and patterns harvested from real campaigns. Pure reasoning skill — no
  scripts. Loads automatically whenever a task requires outreach copy.
tags: [outreach]
---

# Email Drafting

A reasoning-only skill for writing cold emails. No tools, no scripts — just frameworks, patterns, and proven examples from campaigns that actually produce replies.

## When to Use

Trigger this skill when:
- The user says "write a cold email", "draft outreach", "help me with email copy", "write a sequence"
- Any task needs cold email copy — subject lines, full sequences, or single emails

## Phase 0: Intake

Pull the full campaign context before writing anything. Ask everything up front, grouped by category. Skip whatever's already answered.

### Campaign Context
1. What product/service are you selling?
2. What problem does it solve? Who feels that pain most acutely?
3. What's the angle? (hiring signal, competitor displacement, pain-based, event-triggered, etc.)
4. Is there a specific signal or trigger? (job posting, G2 review, LinkedIn engagement, funding round, etc.)

### Audience
5. Who's receiving this? (title, seniority, department)
6. What keeps them up at night? (daily frustrations your product touches)
7. What objections will they have? (budget, switching cost, "we already have X", timing)

### Proof & Credibility
8. What social proof can you cite? (customer logos, case studies, metrics)
...
12. Who's the sender? (founder, AE, SDR — this shapes the voice)
13. Any brand guidelines or words to avoid?

### Sequence
14. How many touches? (default: 3)
15. What's the CTA you want? (call, demo, reply, resource download)
...
Every cold email rides this skeleton:
...
Pick the framework that matches the campaign's angle:
...
Pick based on campaign size and expected ROI per lead:
...
| **Tier 3 (Deep)** | References a specific signal (their post, comment, job posting, news). Unique per lead. | 1-50 leads | 8-20% |

### Subject Line Patterns

Eight battle-tested patterns:
...
|-------|--------|---------|--------|-------|
| **Touch 1** | Day 1 | Hook + proof + soft CTA | 50-90 words | The only email that's allowed to run long |
| **Touch 2** | Day 3-5 | New angle or asset | 30-50 words | Different proof point, not a "bump" |
| **Touch 3** | Day 7-10 | Different proof point or social | 20-40 words | Shorter wins this late |
| **Touch 4** | Day 14-21 | Breakup (optional) | 20-30 words | Drop pressure, leave the door open |

**Sequence rules:**
- Never recycle the same CTA across touches
- Each touch needs a fresh reason to reply
- Later touches = shorter emails
- Never send a "just checking in" or "bumping this" — add value or stop

### Hard Rules

These are non-negotiable. Every email passes all 10:

1. **No filler openers.** Never "I hope this finds you well", "I hope you're having a great week", "just reaching out"
2. **No "just checking in" follow-ups.** Each touch adds a new reason to reply
3. **Max 4 paragraphs per email.** Most are 2-3
...
7. **No filler words.** Banned list: synergy, leverage, circle back, loop in, touch base, align, ping
...
10. **Sign off simply.** Name only, or Name + a one-line title. No "Best regards", no "Looking forward to hearing from you"

## Phase 2: Review & Refine

1. Present 3-5 Touch 1 drafts with different angles/frameworks
2. Ask the user to pick a direction or remix elements
3. Build the full sequence from the chosen direction
4. Cap iteration at 3 rounds — after that, ship

## Example Library

Real emails from real campaigns. Treat them as structural templates — swap product, proof, and signal for the current campaign.

### Type 1: Signal-Based (Hiring)

**Signal source:** LinkedIn Jobs — company posting for a role that the product replaces
...
**Signal source:** Industry association member directory
...
**Signal source:** LinkedIn Sales Navigator — engaged with a competitor's company page content
**Framework:** Signal-Proof-Ask
...
**Why it works:** Acknowledges their research without naming the competitor (save that for LinkedIn). "Other side of that conversation" is intriguing. Proof point (8 of top 10) is specific and strong.

---

### Type 3b: Signal-Based (Competitor Engagement — Commenter)

**Signal source:** Same as 3a, but the lead actually commented (higher intent)
...
**Why it works:** Quotes the exact comment (Tier 3 personalization). "Actively looking at this" matches the behavior. Even shorter than 3a because higher-intent leads need less convincing.
...
**Why it works:** Quotes their own words back to them (the strongest personalization). Empathetic tone — no hard sell. Short. The reactor version is softer because the signal is weaker.
...
**Why it works:** Validates the frustration without bashing the competitor. Names companies they'd respect. Explains the "why" behind the switch (Prolific-verified participants). CTA is specific (20 minutes, comparison framing).
...
**Why it works:** Leads with a specific customer story and hard numbers. Each touch adds new value (case study → ROI model → benchmark offer). Breakup touch removes pressure. CTA escalates: overview → walk-through → benchmark.
...
When delivering email drafts, ship them in this structure:

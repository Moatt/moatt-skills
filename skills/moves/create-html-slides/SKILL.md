...
description: Build stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files. Use when the user wants to assemble a presentation, port a PPT/PPTX to the web, or create slides for a talk or pitch. Helps non-designers find their aesthetic through visual exploration rather than abstract choices.
---

> **Deprecated:** This skill has been superseded by `moatt-graphics`. See `skills/composites/moatt-graphics/` (install with `npx moatt install moatt-graphics`). The slides format is one of seven formats in the newer skill and adds 36 style presets plus image sourcing and PNG export. Kept for one release cycle before being removed.

# Frontend Slides Skill

Build zero-dependency, animation-rich HTML presentations that run entirely in the browser. The skill helps non-designers find a preferred aesthetic through visual exploration ("show, don't tell"), then ships production-quality slide decks.

## Core Philosophy

1. **Zero Dependencies** — Single HTML files with inline CSS/JS. No npm, no build tools.
2. **Show, Don't Tell** — People don't know what they want until they see it. Generate visual previews instead of abstract choices.
3. **Distinctive Design** — Steer clear of generic "AI slop" aesthetics. Every presentation should feel custom-crafted.
4. **Production Quality** — Code must be well-commented, accessible, and performant.
5. **Viewport Fitting (CRITICAL)** — Every slide MUST fit exactly inside the viewport. Never any scrolling within slides. Non-negotiable.
...
To guarantee viewport fitting, enforce these per-slide limits:
...
- User wants to build slides from scratch
- Move on to Phase 1 (Content Discovery)

**Mode B: PPT Conversion**
- User has a PowerPoint file (.ppt, .pptx) to convert
- Move on to Phase 4 (PPT Extraction)

**Mode C: Existing Presentation Enhancement**
- User has an HTML presentation and wants to refine it
...
If the user has content, ask them to share it (text, bullet points, images, etc.).
...
Most people can't put design preferences into words. Instead of asking "do you want minimalist or bold?", we generate mini-previews and let them react.
...
- Skill generates 3 preview files based on those answers
- User opens previews in browser and picks their favorite
- Best for users without a specific style in mind

**Option B: Direct Selection**
- If the user already knows what they want, they can request a preset by name
...
(If the user picks one, skip to Phase 3. If they want more options, show additional presets or run guided discovery.)

**If "Use client branding"** → Load the client's visual identity:
...
Based on the mood selection, generate **3 distinct style previews** as mini HTML files in a temporary directory. Each preview is a single title slide showing:
...
Drop the previews in: `.claude-design/slide-previews/`
...
def extract_pptx(file_path, output_dir):
    """
    Pull every piece of content out of a PowerPoint file.
...
Show the extracted content to the user:
...
Move on to Phase 2 (Style Discovery) with the extracted content in hand.

### Step 4.4: Generate HTML

Convert the extracted content into the chosen style, preserving:
- All text content
- All images (referenced from the assets folder)
- Slide order
- Any speaker notes (as HTML comments or in a separate file)
...
**Fonts not loading:**
- Check the Fontshare/Google Fonts URL
- Make sure font names match in CSS

**Animations not triggering:**
- Verify the Intersection Observer is running
- Check that the `.visible` class is being added

**Scroll snap not working:**
- Ensure `scroll-snap-type` is on html/body
- Each slide needs `scroll-snap-align: start`

**Mobile issues:**
- Disable heavy effects at the 768px breakpoint
- Test touch events
- Drop the particle count or disable canvas
...
6. User picks Style B (Neon Cyber), asks for a darker background
7. Skill generates the full presentation with all slides
8. Skill opens the presentation in the browser
...
2. Skill extracts content and images from the PPT
3. Skill confirms extracted content with the user
4. Skill asks about desired feeling/style
5. Skill generates style previews
6. User picks a style
7. Skill generates the HTML presentation with assets preserved

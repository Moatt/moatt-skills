...
name: kol-discovery
description: >
  Surface Key Opinion Leaders (KOLs) in a given domain by pairing web research with
  LinkedIn post search. Given a company/idea and target domain, the skill generates
  authority keywords, scrapes LinkedIn posts to identify prolific authors with strong
  engagement, then merges those with web-researched influencers. Use whenever the
  request is "find influencers in X space" or "who are the KOLs for Y industry."
tags: [outreach]
---
...
Surface Key Opinion Leaders in any domain by mining LinkedIn posts for prolific, high-engagement authors and merging them with web-researched influencers.

**Core principle:** Search for **authority/thought-leadership keywords**, not pain language. The targets are the people shaping the conversation — conference speakers, newsletter writers, podcast hosts, prolific LinkedIn posters.
...
3. Who's the target audience? (the people the KOLs influence)
4. Any KOLs you already know? (LinkedIn URLs — these become the baseline)
5. Anyone to EXCLUDE? (competitors, your own team, irrelevant voices)

## Phase 1: Generate Domain Keywords

Based on the intake, generate 15-25 topic/authority keywords. These are NOT pain language — they're the terms thought leaders use when sharing expertise:
...
- **Vendor exclusion keywords** — titles to drop (software engineer, recruiter, saas)
- **Domain relevance keywords** — core industry terms for relevance scoring

**Present the keywords for approval before running.**

Save the config in the current working directory or wherever the user prefers:
...
## Phase 2: Run the KOL Discovery Pipeline
...
- `--output-dir` — directory for the output CSV (default: current working directory)
- `--test` — cap at 5 keywords (validation run)
- `--web-kols` — path to web-researched KOL JSON (agent generates this)
- `--yes` — skip cost confirmation prompts
- `--max-runs` — override the Apify run limit
...
**Cost estimate:** ~$0.10 per keyword. A full 20-keyword run lands around $2-3.

**Always run with `--test` first.**

## Phase 2b: Web Research (Agent-Driven)

In parallel with or before the script, do web research to find known KOLs:
- Search "top [industry] influencers on LinkedIn"
...
Pass to the script via `--web-kols`.
...
- **Too few results** — lower `min_posts` or `min_total_engagement`

...
description: >
  Research LinkedIn profiles and write personalized messages for any LinkedIn
  message type — connection requests, InMails, DMs, message requests, post
  comments, and comment replies. Takes LinkedIn URLs as input, researches
  each person (profile data + recent posts via Apify), and generates messages
  tailored to each lead's background, interests, and recent activity. Exports
  tool-ready CSVs for Dripify, Expandi, Botdog, PhantomBuster, or a generic
  format. No LinkedIn cookies or login required.
tags: [outreach, social]
---

# LinkedIn Message Writer

Research LinkedIn leads and write personalized messages for any LinkedIn message type. Takes LinkedIn URLs, researches each person using Apify (profile + recent posts), and writes messages from what it finds.

No LinkedIn cookies. No database setup. LinkedIn URLs in, personalized messages out.
...
No LinkedIn cookies, login, or session tokens needed. Apify handles the scraping without any LinkedIn credentials.
...
- MUST stay under the character limit. Count every character. If you go over, rewrite from scratch — never truncate.
- Free accounts: 200 chars. Premium/Sales Navigator: 300 chars. Ask which they have.
...
- Higher commitment ask is fine here — you're spending a premium credit.
...
- Good for follow-up sequences after the connection is accepted.
...
- More casual than InMail because you have something in common.
...
- This is a warm-up move, not a pitch. The goal is getting noticed before connecting.
...
2. How many leads? (affects cost estimate and whether to use the post scraper)
...
7. Which tone? Present options:
   ...
   - **Thought Leader** — Lead with insight or a contrarian take. Position the sender as expert.
...
8. Any reference messages that have worked well? (these override the tone presets)
...
- **CSV file:** Read the CSV. Look for a column with LinkedIn URLs (common names: `linkedin_url`, `LinkedIn URL`, `LinkedIn`, `profile_url`, `url`). If ambiguous, ask the user which column.
- **Pasted URLs:** User pastes LinkedIn URLs directly. Parse them.
- **Pasted list:** User pastes names + companies or other data. Extract whatever's available.
...
Present the lead count to the user and confirm before moving on to research.

### Phase 2: Research

Research each lead with two Apify actors. Both require only `APIFY_API_TOKEN` — no LinkedIn cookies.

#### Step 1: Profile Data

Use `harvestapi/linkedin-profile-scraper` to pull profile data for all leads.
...
Use `harvestapi/linkedin-profile-posts` to pull recent posts. Run this when:
- User asks for deep personalization
- Lead count is small (under 50) and the budget allows
- User explicitly wants to reference what leads are posting about

Skip it when:
- Lead count is large (100+) and the user prefers speed over depth
...
After research wraps up, present a summary table:
...
If the user asked to filter/qualify leads, do that now based on profile data (title, company, industry, etc.) and report which leads made the cut.
...
Use the strongest available signal for each lead. In order of strength:
...
3. **Creator topics/hashtags** — Reference what they post about more broadly.
...
If the user provided reference messages that have worked, analyze them for tone, length, structure, and vocabulary. Use them as the template — don't override with defaults.

#### Writing Process

1. **Generate samples first.** Write messages for 3-5 leads at different signal-richness levels. Present to the user.
2. **Iterate.** User reviews, gives feedback. Adjust tone/approach. Max 3 rounds.
3. **Batch generate.** Once approved, write messages for all remaining leads.
...
- For connection requests (200/300 chars), every character counts. Be ruthless.
...
- One row per lead with every message in its own column
...
Show 3-5 sample messages from the export for one last review.
...
- Remind the user to verify the first few messages after import
...
| Apify run fails or times out | Retry once. If still failing, skip that lead and note it. |
| LinkedIn URL is invalid or profile not found | Skip the lead, report it to the user |
| 0 profiles returned | Check URL format — must be the full LinkedIn URL with `https://` |

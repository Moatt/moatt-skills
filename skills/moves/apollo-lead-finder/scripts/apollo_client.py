...
Supports a proxy+fallback pattern:
  - If MOATT_API_KEY is set, routes requests through the Moatt proxy.
...
if MOATT_API_KEY:
    BASE_URL = f"{MOATT_API_BASE}/v1/proxy/apollo"
else:
    ...
    def __init__(self, api_key=None):
        ...
        Args:
            api_key: APOLLO_API_KEY from Apollo settings.
                     Optional when running under Moatt (key is managed server-side).
        """
        ...
        if MOATT_API_KEY:
            self.headers["Authorization"] = f"Bearer {MOATT_API_KEY}"

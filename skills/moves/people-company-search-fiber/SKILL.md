...
**Cheaper alternative for people search:** Apollo `mixed_people/search` costs **$0.01 flat** per call regardless of result count. Use `$MOATT_API_BASE/v1/proxy/apollo/mixed_people/search` when possible.

**Tip:** Use the dedicated proxy route `$MOATT_API_BASE/v1/proxy/fiber/...` instead of the generic orthogonal proxy for cleaner billing tracking.

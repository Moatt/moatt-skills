...
name: tam-builder
description: >
  Build and maintain a scored Total Addressable Market using Apollo Company Search.
  Discovers companies matching the ICP, scores fit (0-100), assigns tiers (1/2/3),
  and auto-generates a persona watchlist for Tier 1-2 accounts via Apollo People
  Search (free). Outputs to CSV.
tags: [lead-generation]
---

# TAM Builder

Build and maintain a scored Total Addressable Market. Drives Apollo Company Search to discover companies, scores ICP fit (0-100), assigns tiers (1/2/3), and auto-builds a persona watchlist for Tier 1-2 accounts via Apollo People Search (free).

**Three modes:**
- **build** — First-time TAM construction from an Apollo search
- **refresh** — Update an existing TAM: re-score, detect tier changes, deprecate stale companies
- **status** — Read-only report on the current state of the TAM
...
2. Score them and present: tier distribution, sample Tier 1/2 companies, scoring sanity check
...
Step 2: User reviews the sample → approves, adjusts filters, or caps scope
...
- `industry_fit` — industry matches the targets?
...
- **People Search:** `POST https://api.apollo.io/api/v1/mixed_people/search` — **$0.01 flat per call** (cheapest people search). Returns matching people in the `people` array. Fields: `first_name`, `title`, `organization.name`. Email/LinkedIn obfuscated on the free tier.
...
- **Pagination:** `per_page` (max 100), `page` (1-indexed). `pagination.total_entries` gives the total.

## Output

Save results as CSV in the current working directory:
- `tam-companies-{date}.csv` — Every discovered company with ICP score and tier

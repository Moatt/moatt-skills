---
name: moatt-graphics-create-format
description: >
  Creates a new graphics format spec (custom canvas dimensions + content rules)
  and publishes it to the Moatt library so any agent can render against it.
  ...
  Mirrors moatt-graphics-create-style but for formats.
tags: [design, content]
---

# Create Moatt Graphics Format

Authors a new graphics format and publishes it to the central Moatt library
via `npx moatt formats publish`. The output is a working directory with a
`moatt-format.json` manifest plus at least one rendered example PNG paired
...
code change to the `moatt-graphics` skill pack's `FORMAT_CONFIGS` and is
out of scope for this skill. Suggest the closest existing canvas
instead.

**Always check first:** run `npx moatt formats list` (or `formats search
"banner"` etc.) to see whether a community-published format already covers the
use case. If one fits, just use it via the regular `moatt-graphics` flow.

## Prerequisites

- The `moatt-graphics` skill must be installed in the same workspace —
  this skill uses `moatt-graphics/screenshot/screenshot.js` to render
  examples and pulls style specs via `npx moatt styles get <slug>`.
  Install via:
  ```bash
...
- The screenshot tool's dependencies must be installed
  ...
  ```
- The user must be signed in to moatt (`npx moatt login`) for publish
  to authenticate.
...
```
<working-dir>/
  moatt-format.json    # manifest (see "Manifest format" below)
...
`moatt-graphics` skill pack's `FORMAT_CONFIGS` and is out of scope for
...
**Collision check:** run `npx moatt formats get <slug>` — if the catalog
...
1. Pick a style: `npx moatt styles list | head` and choose 2–3 with
   contrasting moods (e.g., `matt-gray`, `neon-dashboard`, `brutalist`).
2. Fetch its slim spec: `npx moatt styles get <slug>`.
3. Generate HTML at the **exact** target dimensions, using the chosen style's
   ...
   `moatt-graphics` rendering pipeline apply:
...
4. Render via the screenshot tool from the `moatt-graphics` skill pack,
...
### Step 5 — Write `moatt-format.json`

Match the shape documented in `moatt-graphics/SKILL.md` §17.2. Required:
...
The CLI reads `moatt-format.json`, validates it client-side, uploads the
...
https://skills.moatt.com/formats/<slug>
```
...
- How any agent can now use it:
  ```
  /moatt-graphics --style <any-slug> --format <new-slug> --brief "..."
  ```
...
- **Don't publish without checking `npx moatt formats list` first** — if

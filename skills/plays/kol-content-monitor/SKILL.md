...
description: >
  Track what key opinion leaders (KOLs) in your space are posting on LinkedIn and
  Twitter/X. Surfaces trending narratives, high-engagement topics, and early signs
  of conversations before they peak. Chains linkedin-profile-post-scraper and
  twitter-mention-tracker. Use when a marketing team wants to ride trends rather
  than start them from scratch, or when a founder wants to know which topics are
...
Track what Key Opinion Leaders in your space are writing about. Surface trending narratives early — before they peak — so your team can join the conversation at the right moment with relevant content.

**Core principle:** For seed-stage teams, the fastest path to content distribution is riding a wave that's already breaking, not building one from scratch.
...
### KOL List
1. Names and LinkedIn URLs of the KOLs to track (if known)
   - If unknown: use `kol-discovery` to build the list first
2. Twitter/X handles for the same KOLs (optional but recommended for a fuller picture)
3. Any specific topics/keywords you care about? (for filtering noisy feeds)

### Scope
4. How far back? (default: 7 days for the weekly monitor, 30 days for the first run)
5. Minimum engagement threshold to include a post? (default: 20 reactions/likes)

Save the config to the current working directory as `kol-monitor.json` (or user-specified path).
...
Run `linkedin-profile-post-scraper` for every KOL LinkedIn profile:
...
Filter results: only keep posts with reactions ≥ `min_reactions`.
...
Filter: only keep tweets with likes ≥ `min_reactions / 2` (Twitter engagement runs lower than LinkedIn).

## Phase 3: Topic Clustering

Group every post across every KOL by topic/theme:
...
4. Rank by: total engagement (sum of reactions/likes across every post on that topic)

That surfaces topics with **broad consensus** (multiple KOLs discussing it) vs. individual takes.

### Signal types to flag:

| Signal | Meaning | Example |
|--------|---------|---------|
| **Convergence** | 3+ KOLs on the same topic in the same week | Multiple founders posting about "AI SDR fatigue" |
| **Spike** | Topic volume 2x'd vs last week | Suddenly everyone's talking about [new thing] |
| **Underdog** | 1 KOL posting about a topic nobody else covers | Potential early-mover opportunity |
...
Optional: from the monitor output, propose a content calendar entry for every "Ride the Wave" opportunity:
...
Run weekly (Friday afternoon — catches the week's peaks and gives the weekend to draft):
...
- **Optional upstream:** `kol-discovery` (to build the initial KOL list)

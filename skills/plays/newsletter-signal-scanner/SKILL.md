...
description: >
  ...
  AgentMail, matches them against keyword campaigns, and delivers a weekly digest
  of actionable signals. Use when a marketing team wants to turn newsletter
  subscriptions into an ongoing intelligence feed without reading them manually.
tags: [monitoring]
---

# Newsletter Signal Scanner

Turn newsletter subscriptions into a structured intelligence feed. Monitors an AgentMail inbox for incoming newsletters, pulls out signal-relevant content per keyword campaign, and delivers a weekly digest of what matters — competitor mentions, ICP pain language, market shifts, and emerging topics.
...
Save the campaign config to the current working directory as `newsletter-signals.json` (or user-specified path).
...
For each matched email, pull clean signal snippets:
...
For first-time setup, subscribe the AgentMail address to the target newsletters:

1. Get the AgentMail inbox address (via the AgentMail API at agentmail.dev)
2. For each newsletter, visit the subscription page and submit the AgentMail address
3. Confirm subscriptions (check the inbox for confirmation emails)
4. Allow 1-2 weeks of accumulation before the first full digest
...
- **AgentMail API** (agentmail.dev) — for inbox access. Requires the `AGENTMAIL_API_KEY` environment variable and the `agentmail` pip package (`pip3 install agentmail`).

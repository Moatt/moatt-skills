...
version: 1.0.0
description: >
  End-to-end composite for champion/buyer/user job-change signals. Takes a set of
  ...
  personalized outreach that leans on the existing relationship. Tool-agnostic —
...
graph:
  provides:
    ...
    - personalized-email-sequences  # Outreach drafts that lean on the existing relationship
  requires:
...
Tracks known buyers, champions, and power users for job changes. When someone who already knows your product moves into a new role, you have the highest-conversion outbound signal in B2B — they already trust you, they're trying to make an early impact at the new gig, and they have firsthand experience with what your product delivers.
...
- They have a positive past experience to anchor on
- They want quick wins at the new company (bringing in a tool they trust IS a quick win)
- They can champion the deal internally with firsthand results to cite

## When to Auto-Load

Load this composite when:
- The user says "track my champions", "check for job changes", "who moved from our customer accounts", "champion tracking"
- The user has a list of past buyers/users and wants to know who's changed jobs
...
| **Past champions** | Advocated for your product internally, drove adoption | Will champion again. Often hold stronger conviction than the original buyer. | `past_champions` |
...
| Any specific results from the tracked person's previous company? | The strongest possible proof | `specific_results` |
...
**Purpose:** for each person in the tracking list, determine whether they've moved to a new company.
...
tracked_people: [
  {
    ...
    email: string | null          # Previous email (will be outdated after the move)
...
1. **Check current position** against `last_known_company`:
   - **LinkedIn:** visit the profile (or use a scraping tool) → compare current company to `last_known_company`
   - **Apollo:** people search by name + LinkedIn URL → check current employer
   - **Web search:** `"{full_name}" AND ("{last_known_company}" OR "joined" OR "new role")` — hunt for announcements
...
2. **For each person, determine:**
   ...
   - Have they moved? → signal detected. Capture the new company + new title.
   - Has their profile gone dark (no updates, no activity)? → flag for manual check.

3. **For each mover, extract:**
   - New company name
   - New title
   - Approximate start date at the new company
   - New company domain (for research in Step 2)
   - Was this a promotion (higher title) or a lateral move?
...
**Purpose:** for each mover, research their new company and decide whether it's a fit for your product. This is the critical gate — just because someone you know moved doesn't mean their new company is a prospect.
...
2. **Qualify against ICP:**
   ...
   | Size | Is employee count within `target_company_size`? | Hard filter |
...
3. **Assess the person's position at the new company:**
   ...
   | **Department fit** | Are they in a department that buys/uses your product? | They have to be in the right department |
...
- **Tier 1 (Act Today):** past buyer or champion + strong ICP fit + <30 days in the new role. The warmest possible lead.
...
- **Disqualify:** new company doesn't pass the ICP hard filters.
...
**Purpose:** get the mover's new email and any additional context at the new company.
...
1. **Find the new work email** — their old email is outdated. Use the configured `contact_tool`:
...
3. **Flag contacts without email** — they should be routed to LinkedIn outreach instead.
...
**Purpose:** draft outreach that leans on the existing relationship. This is NOT cold email — these people know you. Tone, length, and approach are fundamentally different from the other signal composites. Pure LLM reasoning — inherently tool-agnostic.
...
4. **Sequence design (warmer, more spaced):**
   ...
   **Key difference from cold sequences:** more spacing between touches (they're not a stranger you'll lose by waiting), warmer tone, and Touch 3 is a check-in, not a breakup.

5. **Follow `email-drafting` hard rules** with one exception: Rule 6 ("never lie about how you found them") flips — you SHOULD reference exactly how you know them. That's the whole point.
    ...
    sequence: [
      {
        ...
        personalization_elements: {
          ...
          new_company_relevance: string   # Why the new company is a fit
        }
        word_count: integer
      }
    ]
...
Identical to `funding-signal-outreach` Step 5. Package the contacts + email sequences for the configured outreach tool. Route LinkedIn-only contacts to the `linkedin-outreach` skill.

### Additional Note for This Composite

Some contacts will be email, others LinkedIn-only. Split the output:
...
| **Qualification** | Is the company relevant? | Is the NEW company relevant? (Person already qualified) |
...
- **Run this check monthly, not daily.** Job changes don't happen overnight. Monthly cadence catches moves within the first 30 days — the optimal outreach window.
- **Keep your tracking list fresh.** Add new champions, buyers, and power users as deals close. Drop people who've left the industry entirely.
...
- **Promotions are the best sub-signal.** Someone who championed you and is now a VP is the warmest possible lead — they already believe in your product AND now control budget.

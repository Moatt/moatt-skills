...
version: 1.0.0
description: >
  Full-cycle funding signal composite. Takes any set of companies, detects
  recent funding events, qualifies them against your company context, finds
  relevant people (buyers, champions, users), and drafts personalized
  outreach. Tool-agnostic — works with any company source, contact finder,
...
Catches recent funding events across a set of companies, qualifies them against your company's context, surfaces the right people to reach out to, and drafts personalized emails. The full chain from signal to outreach-ready.
...
1. **Search for funding announcements** using the configured `signal_tool`:
   ...
   - **Apollo:** Use the company enrichment endpoint to pull funding data
   - **Crunchbase:** Query the funding rounds API filtered by date
...
After approval, generate the remaining emails and output the full set.
...
- Chain to the `linkedin-outreach` skill for LinkedIn message sequences
- Output CSV for the LinkedIn automation tool (Dripify, Expandi, etc.)

...
description: >
  ...
  create-x-content + create-linkedin-content + moatt-graphics in one invocation.
  Asks about the graphic format when not specified, with heuristic-driven recommendations.
tags: [content, social]
---

# Social Kit

One command → X variants + LinkedIn variants + a graphic. A thin orchestrator that chains `create-x-content`, `create-linkedin-content`, and `moatt-graphics`, with sensible defaults and a pre-flight check for voice guides.
...
Check `~/.moatt-skills/config.json` for `voice_guides.x` and `voice_guides.linkedin`. If either is missing and that platform isn't being skipped:
...
Otherwise, default to `matt-gray` unless the user stops the skill to pick another. To change the default per-user, a future enhancement can add a `style_default` key to `~/.moatt-skills/config.json`.

### Step 7 — Generate the graphic

Invoke `/moatt-graphics` with:
```
--style <style>
--format <format>
...
Point the output at `<resolved-output>/graphic/`. The distilled brief should include:
...
Voice guides + config at `~/.moatt-skills/voice-guides/` and `~/.moatt-skills/config.json` are discovered automatically.

## Examples

**Typical run (format asked):**
```
/social-kit --brief "Claude can now verify email deliverability for $0.01 per check. Install moatt and ask. Runs MX, SMTP, RCPT TO, catch-all, disposable, blocklist checks."
```
...
- **First-time users:** run `/generate-voice-guide` once before your first `/social-kit`. One-time setup.
...
- **Iterate on voice before iterating on content.** If drafts feel off, 80% of the fix is re-running `/generate-voice-guide` with more recent posts or more iterations — not rewriting the skill prompts.

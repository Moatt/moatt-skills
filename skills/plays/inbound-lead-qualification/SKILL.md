...
version: 1.0.0
description: >
  Qualify inbound leads against full ICP criteria — company size, industry, use case
  fit, role/seniority of the person. Checks CRM and existing customer base for
  duplicates and current relationships. Emits a scored CSV with qualification status,
  reasoning, and pipeline overlap flags. Tool-agnostic — works with any CRM,
  enrichment tool, or data source.
tags: [lead-generation]
---

# Inbound Lead Qualification

Takes a set of inbound leads and validates each one against your full ICP criteria. Not a fast-pass triage (that's `inbound-lead-triage`) — this is the deeper qualification step that determines whether a lead is genuinely worth pursuing, and emits a scored CSV for the team.

## When to Auto-Load

Load this composite when:
- User says "qualify these inbound leads", "check if these leads are ICP", "score my inbound"
- An upstream triage has finished and the leads now need deeper qualification
- The user has a batch of leads and wants a qualified/disqualified verdict on each
...
On the first run, set the ICP definition and CRM access. Save to the current working directory or wherever the user prefers (e.g., `config/lead-qualification.json`).
...
**If the `lead-qualification` capability already has a saved qualification prompt:** Use it directly — don't rebuild ICP criteria from scratch.

**On later runs:** Load the config silently.
...
### Process
1. Load the client's ICP config (or qualification prompt from the `lead-qualification` capability)
2. Parse the inbound lead list — accept any format:
   - Output from `inbound-lead-triage` (already normalised)
   - Raw CSV with any column structure
   - Pasted list of names/emails/companies
   - CRM export
3. Identify what data is present vs missing per lead:
   - **Have:** Fields included in the input
...
### Output
- Parsed lead list plus inventory of available/missing fields
- Gap report for the user

### Human Checkpoint
If >50% of leads lack critical fields (company name or person title), recommend running `inbound-lead-enrichment` first. Ask: "Many leads are missing company/title data. Want me to enrich them first, or qualify with what's available?"
...
- Search the customer database by company domain/name
- If matched: Flag as `existing_customer` with customer details (plan, account owner, contract status)
...
- If matched: Flag as `in_pipeline` with deal details (stage, owner, last activity)
- Critical: Sales reps need to know before reaching out that a colleague already owns the account

**Check 3 — Previous engagement?**
- Search outreach logs for the email/company
- If matched: Flag as `previously_contacted` with history summary (when, what channel, outcome)

**Check 4 — Known from signal composites?**
- Search your CRM or signal tracking system for the company
- If matched: Flag as `signal_flagged` with signal type and date

### Output
Each lead tagged with:
- `pipeline_status`: `new` | `existing_customer` | `in_pipeline` | `previously_contacted`
- `pipeline_detail`: One sentence describing the overlap (or null)
- `signal_flags`: Any signal composite matches

### Handling Overlaps
- **Existing customer:** Don't disqualify. Mark separately. May be expansion/upsell.
- **In pipeline:** Don't disqualify. Flag for sales rep coordination. Note the existing deal owner.
- **Previously contacted but no response:** Still qualify. The inbound signal means they're warmer now.
...
- Check employee count against the ICP range
...
- Be intelligent about classification: "AI-powered HR platform" matches both "AI/ML" and "HR Tech"
...
- Check the HQ location and/or the person's location
- For remote-first companies, look at where the majority of the team sits
- Score: `match` | `borderline` | `mismatch` | `unknown`

**Dimension 5 — Use Case Fit**
- Given what the company does, could they plausibly use the product?
- This is the most nuanced dimension — demands understanding of both the product and the company's operations
- Sources: company website, product description, job postings (which hint at internal tools/processes)
- Score: `strong_fit` | `moderate_fit` | `weak_fit` | `no_fit` | `unknown`

### Output
Each lead receives a `company_qualification` block:
```
{
...
- Check the title against the buyer persona title lists
- Handle variations: "VP of Marketing" = "Vice President, Marketing" = "VP Marketing"
- Account for title inflation at small companies (a "Director" at a 10-person startup ≠ "Director" at a 10,000-person enterprise)
...
- If the company qualifies but the person doesn't: Flag as `right_company_wrong_person` — this is a referral opportunity
- If the person qualifies but the company doesn't: Flag as `right_person_wrong_company` — rare for inbound, but possible with job changers

### Output
Each lead receives a `person_qualification` block:
```
{
...
### Process
This step ties the company's likely needs to your product's actual capabilities. It runs deeper than Step 3's company-level use case check.
...
2. **Map intent to product capabilities:**
   - Does the product actually solve what they appear to need?
   - Is this a primary use case or a stretch?
   - Any known limitations that would disappoint them?

3. **Assess implementation feasibility:**
   - Given company size and stage, can they realistically implement?
...
Combine every dimension into a final qualification verdict.
...
- Any hard qualifier present → `qualified` regardless of score (but still surface the full breakdown)
- Existing customer → Route separately; don't score as a new lead
...
- `borderline_review` — Score 50-74, requires a human judgement call
...
- `disqualified_polite` — Score < 30, needs a polite decline
...
Produce a CSV that preserves ALL input fields and appends the qualification columns:
...
**Use case columns:**
- `inferred_intent` — What they appear to need
- `product_fit` — strong | moderate | weak | unknown
- `implementation_feasibility` — easy | moderate | complex | unlikely
...
- Extract the company domain from the email
- If a corporate domain: look up the company and proceed with company qualification (person qualification will lean "unknown")
- If a personal email (gmail, yahoo): Score as `insufficient_data`, recommend enrichment or manual review

**Same company, multiple leads:**
- Qualify the company once, apply to every lead from that company
- Run person qualification individually for each
- Flag the multi-contact opportunity: "3 people from [Company] came inbound — potential committee buy"

**Contradictory signals:**
- Company is strong fit but the person is completely wrong (e.g., intern at a perfect company)
...
- Reasoning: they came to YOU. The intent signal tips borderline cases toward "worth a conversation"
- Note the lean in the reasoning: "Borderline on [dimension], but inbound intent suggests pursuing"

**Scoring with missing data:**
- Unknown dimensions score 30 (not 0, not 50) — missing data is mildly negative but not disqualifying
...
- **Enrichment tools** — SixtyFour or Orthogonal, LinkedIn scraper, or similar (optional, boosts accuracy)

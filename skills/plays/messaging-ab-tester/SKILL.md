...
description: >
  Produce 3-5 messaging variants for a value proposition, design structured A/B tests,
  and analyze the results to determine which framing resonates most with the ICP. Tests
  can run via LinkedIn organic posts, cold email subject-line splits, or both. Pure
  reasoning for variant generation and analysis — the user deploys the tests through
  their own tools. Use when a team can't decide between messaging angles and needs
  data, not opinions.
tags: [brand]
---
...
Stop debating which message is better — test it. Produce messaging variants, deploy them through real channels, and measure which framing actually resonates with your ICP.
...
5. **Where to test:**
   ...
   - **Both** — run in parallel for the fastest signal
...
Produce 3-5 variants that test different **angles**, not just different words. Each variant should represent a separate strategic bet:
...
3. Don't boost any posts — organic only for a clean comparison
...
1. Create a campaign with every variant as an A/B test sequence
2. Split the list evenly across variants (minimum 50 per variant for signal)
...
- Click rate (if a link is included)

### Option C: Both (Recommended)

Run LinkedIn and email in parallel. Different channels may produce different winners — that's valuable signal about where each message works best.

## Phase 2B: Collect Results

Once the test has run for the planned duration, gather your results:
...
**For LinkedIn tests:** open your post analytics (click "View analytics" on each post) and share impressions, reactions, comments, and profile visits per post.
...
- **LinkedIn organic** — post variants manually or via a scheduling tool

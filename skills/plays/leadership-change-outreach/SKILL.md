...
version: 2.0.0
description: >
  Full-cycle leadership-change signal composite. Takes any set of companies,
  catches recent leadership changes (new VP+ and C-suite hires plus
  promotions), assesses relevance to your product, and drafts personalized
  outreach. Uses Apollo People Search (free) for fast detection plus Apollo
  Enrichment (1 credit/person) for employment history, start dates, LinkedIn
  URLs, and verified emails.
tags: [outreach]
...
Catches new leadership hires at target companies and assesses whether the new leader is relevant to your product — as a direct buyer, a champion, or someone whose mandate aligns with what you sell. If relevant, enriches their profile and drafts personalized outreach that speaks to their new-role priorities.
...
This composite uses a two-phase Apollo pipeline that supersedes slower web-search approaches:
...
2. **Local Post-Filter** — Strict title matching to remove noise from Apollo's fuzzy matching (regional titles, sub-function heads, non-GTM roles). Typically cuts results by 50-60%.
...
**Why this beats web search:** Web search depends on press releases and announcements — most leadership changes below C-suite are never publicly announced. Apollo pulls from LinkedIn profile data directly, catching changes web search misses. Speed: ~90 seconds total vs 5+ minutes for web search.
...
On the first run for a client/user, collect and store these preferences. Skip on subsequent runs.
...
This typically cuts results by 50-60% (e.g., 100 raw → 40 filtered).
...
- Every company domain can be passed in a single query (newline-separated)
...
Save two files:
1. **CSV** (`leadership-change-scan.csv`) — every enriched person sorted by start_date descending, with columns: name, title, company, domain, start_date, change_type, previous_title, previous_company, previous_end_date, email, email_status, linkedin_url, city, state, country
...
2. **Build personalization from the enriched profile:**
...
In the funding and hiring composites, the signal is about the company, and you then find people to contact. In leadership change, **the signal IS the person.** The new leader is both the signal and the primary contact. This changes the flow:

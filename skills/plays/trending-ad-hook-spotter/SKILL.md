...
description: >
  Watch Twitter/X, Reddit, LinkedIn, and Hacker News for trending narratives,
  viral posts, and hot-button topics in your space. Maps each trend to a
  concrete ad-hook opportunity with a timing-urgency score. Tells you exactly
  what to run ads about right now while the topic is still hot.
tags: [ads]
---

# Trending Ad Hook Spotter

Scans social platforms for what's trending in your space right now — viral posts, hot debates, breaking news, memes — and translates each trend into a concrete ad hook you can run while the topic is still warm.

**Core principle:** The highest-performing ads ride cultural and industry moments. This skill spots those moments before your competitors and tells you exactly how to capitalize.
...
- **Environment variable:** `APIFY_API_TOKEN` — required for Reddit scraping (optional if you're using only web_search + the HN API)
- **Web search access** — your AI agent must support `web_search` or equivalent for Twitter/X and LinkedIn lookups
- **No API key needed** for Hacker News (the Algolia HN API is free and public)
...
3. **ICP keywords** — 5-10 keywords that describe your buyer's world
4. **Competitor names** — So we can spot when they get caught up in a trend
...
Use web_search with `site:x.com` or `site:twitter.com` to surface trending posts — no scraper or credentials needed:
...
- Industry-level keyword searches to surface viral posts
...
- Check the front page for anything tangentially related

## Phase 2: Trend Identification & Scoring

### Trend Detection Framework

Bundle collected signals into trends. A "trend" is:
...
3. **3 hook variants:**
   ...
   - **Practical hook** — Offer a solution tied to the trend ("[Trend] means you need [your feature] now")
...
| **Total** | **~$0.05-0.10** (or free if you skip the Reddit Apify scraper) |

## Tools Required

- **Environment variable:** `APIFY_API_TOKEN` — for Reddit scraping via Apify (optional — the skill still works without it using a web_search fallback for Reddit)
...
- No third-party libraries needed. All data collection runs over HTTP APIs (`requests` or equivalent) and web_search.

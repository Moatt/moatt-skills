...
version: 1.0.0
description: >
  Fill in the gaps for inbound leads — research the company, identify the person's
  role and seniority, surface the other stakeholders at the company, check for any
  existing CRM relationships, and update the lead record. Produces enriched lead data
  ready for qualification or outreach. Tool-agnostic.
tags: [lead-generation]
---

# Inbound Lead Enrichment

Take inbound leads with incomplete data and fill in the missing pieces. Researches the company, identifies the person's role, surfaces the other stakeholders at the company, and checks for existing relationships in CRM. Turns a bare email address into a full lead profile.

## When to Auto-Load

Load this composite when:
- The user says "enrich these leads", "fill in the missing data", "research these inbound leads"
- `inbound-lead-qualification` flags leads as `insufficient_data`
- `inbound-lead-triage` spots leads missing company/title fields
- The user has a list of emails or partial lead data and needs complete profiles
...
On first run, lock enrichment-tool preferences.
...
- `person_email` — Contact email (usually already have it from inbound)
...
- `person_tenure` — How long at the current company
...
| **Email-only** | Only an email address | Full research from scratch | High |
...
### Process
For each unique company in the lead list (deduplicate — don't research the same company twice across multiple leads):

**From email domain (if company name is missing):**
1. Pull the domain off the email (e.g., `jane@acme.com` → `acme.com`)
...
- **Deep:** every field, multiple sources, cross-verified
...
1. Check whether name + any other available data can identify the company (e.g., form field, chat message)
2. If a company is referenced in their form submission or chat, use that
3. If genuinely unknown, flag as `company_unidentified` — still proceed with person research if a name is available
...
1. Search LinkedIn for the person at the company (via the configured tool or web search)
2. Cross-reference with SixtyFour or Orthogonal
3. If multiple matches, use the email domain to disambiguate
...
**Why this matters:**
- Inbound leads rarely make the call alone
- Finding the rest of the buying committee early speeds up the deal
- Multi-threading (engaging multiple people at a company) dramatically lifts win rates

**Who to find (based on buyer personas from config):**

1. **Economic buyer** — the person signing the check. Usually VP+ or C-level in the relevant department.
2. **Champion** — most likely to push for adoption internally. Usually a senior IC or Director feeling the pain.
3. **Technical evaluator** — assesses the product's technical fit. Usually engineering or ops.
4. **End user** — uses the product daily. Their buy-in prevents post-sale churn.

**Process per company:**
1. Use the buyer personas to decide which roles to search for
2. Search via the configured tool (SixtyFour or Orthogonal, LinkedIn, company-contact-finder)
...
If the inbound lead is unknown → identifying their role drives the multi-threading strategy
...
- Critical: avoid the "we cold-emailed you last week and you ignored us, now you came inbound" collision
...
- Has this company shown up in any signal scans?
...
Merge every piece of research into a single enriched record per lead:
...
Produce a CSV that extends the original lead data with every enriched field:
...
1. Extract the domain → look up the company
2. Search "[name] [company]" on LinkedIn/web
3. If still unidentifiable: flag as `enrichment_failed` with reason, recommend manual lookup
4. Don't burn paid API credits on truly unidentifiable leads — web search first

**Same company appears multiple times (multiple inbound leads):**
- Research the company ONCE, apply to every lead
...
- If the primary tool fails, fall back to the secondary (web search is always available)
...
- Provide a cost estimate before running paid enrichment tools

**Personal email domains:**
- If the form had a company field: use it even though the email is personal
- If no company info: try a LinkedIn search by name
- Last resort: flag as `company_unidentified`, enrich the person only

---

## Update Protocol

After enrichment finishes, update the source systems:

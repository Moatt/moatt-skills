...
description: >
  ...
  support tickets, and competitor ads. Pulls the actual pain language, competitor
  weaknesses, and outcome phrases that real buyers use. Outputs a ranked angle
  bank with proof quotes and recommended ad formats per angle.
tags: [ads]
---

# Ad Angle Miner

Dig through real customer voice — reviews, Reddit, support tickets, competitor ads — and pull out the specific language, pain points, and outcome desires that make ads convert. The output is an angle bank your team can pull from for any campaign.

**Core principle:** The best ad angles don't come from a brainstorm. They're extracted from what real people are already saying. This skill finds those angles and ranks them by strength of evidence.
...
- **Web search access** — the AI agent must support `web_search` or equivalent for Twitter/X and competitor ad lookups

## Phase 0: Intake

1. **Your product** — Name + a one-sentence description
...
- **Review language patterns** — The exact phrases buyers use
...
Use web_search to check the Meta Ad Library for competitor creatives — no separate tool needed:
...
If the user provides support tickets, NPS comments, or sales call transcripts — ingest and tag them with the same framework below.

## Phase 2: Angle Extraction

Run all collected data through this extraction framework:
...
**Total score out of 100. Rank every angle.**
...
Save to `angle-bank-[YYYY-MM-DD].md` in the current working directory (or the user-specified path).

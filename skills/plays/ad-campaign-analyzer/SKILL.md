...
description: >
  Turn raw ad-campaign performance data (Google, Meta, LinkedIn) into clear-cut
  decisions about what's working, what's burning budget, and what to cut/scale/test
  next. Runs statistical analysis, funnel diagnostics, and multi-channel budget
  reallocation with specific dollar-shift recommendations and scenario modeling.
tags: [ads]
---

# Ad Campaign Analyzer

Take raw campaign data and convert it into clear decisions. This skill goes beyond surface-level metric summaries — it diagnoses problems, surfaces winners, runs significance checks, and tells you exactly what to cut, scale, and test next. Then it pushes further: comparing channels on equal terms, identifying where you're over-spending or under-spending versus results, and producing a concrete reallocation plan.

**Core principle:** Most founders glance at their ad dashboard, see a ROAS number, and either panic or celebrate. This skill gives the nuanced analysis a paid-media specialist would: what's actually significant, what's noise, and where the next dollar should land. It also tackles the allocation problem — most startups either thin out budget across too many channels (none gets enough to learn) or pile everything into one (missing cheaper opportunities elsewhere).
...
1. **Campaign data** — one of:
   ...
   - Screenshots of the dashboard (we'll extract the data)
2. **Platform(s)** — Google / Meta / LinkedIn / All
3. **Time period** — what date range does this cover?
4. **Monthly budget** — total ad spend across the period
5. **Primary goal** — which conversion are you optimizing for? (Demos / Trials / Purchases / Leads)
6. **Target metrics** — do you have a target CPA or ROAS? (If not, we'll benchmark)
...
Normalize everything into a single analysis format:
...
When data spans multiple channels, also build a channel-level rollup:
...
*CAC = Full customer acquisition cost if funnel data is provided (CPA × close-rate adjustment)
...
That number exposes which channels produce leads that actually close, not just convert.
...
Spot spend that produced no or negative return:
...
| **Dayparting waste** | Conversions cluster at certain hours; spend is 24/7 | Set an ad schedule |
...
When data spans multiple channels, run cross-channel budget optimization.
...
**Efficiency Index:**
- **> 1.0** = Under-invested (taking more than its share of conversions)
- **= 1.0** = Proportional (fair share)
- **< 1.0** = Over-invested (taking less than its share)

### 4B: Marginal Return Analysis

For each channel, estimate whether additional spend would deliver proportional returns:
...
Save to `campaign-analysis-[YYYY-MM-DD].md` in the current working directory (or wherever the user prefers).
...
- User supplies the campaign data as CSV, paste, or screenshot

...
Topic cluster mapping gets significantly better with keyword data that shows search volume, difficulty, and semantic relationships across hundreds of subtopic variations.
...
>
> "Want to use DataForSEO or a fallback, or should I proceed with baseline mode? Baseline uses our existing SEO tools and web research — still produces a strong topic map, but with less granular volume data per subtopic."

### Mode Selection

- **Recommended mode (DataForSEO)** — Bulk keyword data via DataForSEO API. Pulls search volume, difficulty, and semantic grouping for every subtopic. Enables data-driven prioritisation and precise gap identification. Also supports keyword clustering APIs that automatically group related terms. Falls back to Keywords Everywhere, SEMrush, or Ahrefs if their env vars are set instead.
- **Baseline mode** — Uses `seo-domain-analyzer` for domain metrics, `web_search` for topic research, `reddit-post-finder` for question mining, competitor analysis via `site-content-catalog`. Topic mapping and cluster architecture stay equally strong. Volume estimates are directional, not exact.
...
3. **Competitors** — 2-5 competitor URLs who rank well for those topics
4. **ICP** — Who reads your content? (role, pain, goal)
5. **Content capacity** — How many articles can your team ship per month?
...
Map every piece of existing content:
- Blog posts by topic
- Resource pages, guides, glossary
- Landing pages with content
- Identify the topics you already cover
- Flag: thin pages, outdated content, orphan pages (no internal links)
...
- How are their topic clusters structured?
...
Run `seo-domain-analyzer` for your site and each competitor:

- Your domain authority vs the competitors
...
For each target topic area, build out the full subtopic universe:
...
Use several sources to assemble the subtopic list:
...
Group every discovered keyword/subtopic into semantic clusters:

**Recommended mode (DataForSEO):** Use the DataForSEO keyword clustering API or group by SERP overlap (keywords that share 3+ ranking URLs likely belong to the same cluster).
...
| **Cluster article** | Deep dive on a subtopic | 1,500-3,000 |
...
4. **Cross-cluster links** wherever topics overlap
...
| **Effort** | 15% | How much work to produce high-quality content? |
...
**Month 2:** Deepen Cluster 1, kick off Cluster 2
...
- [Continue the pattern based on capacity]
...
| DataForSEO keyword data (recommended) | ~$0.50-3.00 (depending on keyword count) |
| Fallback tools (Keywords Everywhere, SEMrush, Ahrefs) | ~$0.01-0.10 (if used instead of DFS) |

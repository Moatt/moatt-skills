...
description: >
  Inspect the message match between your ads and landing pages. Confirms whether
  the promise from the ad copy carries into the landing page headline, body, and
  CTA. Flags every disconnect that's costing you conversions. Works across
...
The #1 reason ads earn clicks but no conversions: the landing page fails to deliver on the ad's promise. This skill audits the entire click path — ad copy through landing-page experience — and flags every disconnect.
...
1. **Ad copy** — For each ad, supply:
   - Headline(s)
   - Body / description text
   - CTA text
   - Platform (Google Search / Meta / LinkedIn)
2. **Landing page URLs** — The URL each ad points to
3. **Conversion goal** — What's meant to happen after the click? (Demo / Trial / Purchase / Download)
4. **Known conversion rates?** — Current click → conversion rate per ad/LP (if you have it)
...
If `fetch_webpage` is unavailable, fall back to `curl` to grab the HTML.
...
|-----------|-------------|----------|
| **Promise continuity** | [X] | Does the LP headline keep the ad's promise? |
| **Language match** | [X] | Does the LP echo the words/phrases from the ad? |
| **Visual continuity** | [X] | Does the LP feel like an extension of the ad? (N/A for search) |
| **CTA alignment** | [X] | Does the LP's ask match what the ad implied? |
| **Specificity match** | [X] | If the ad was specific ("for sales teams"), is the LP equally specific? |
| **Emotional match** | [X] | If the ad leaned on fear/urgency, does the LP carry that forward? |
...
| 30-39 | Needs work | Noticeable gaps — visitors must hunt for relevance |
| 20-29 | Poor | Ad and LP look like different products |
| Below 20 | Critical | Complete mismatch — fix immediately |

## Phase 3: Conversion Friction Analysis

Beyond message match, evaluate landing page conversion friction:
...
| **CTA clarity** | One clear CTA or competing actions? | [Clear/Cluttered] |
| **Above-fold conversion** | Can a visitor convert without scrolling? | [Yes/No] |
| **Social proof placement** | Is proof near the CTA? | [Yes/No] |
| **Navigation distraction** | Does the LP carry full site nav? (Should be minimal) | [Minimal/Full nav] |
...
**Disconnect found:** [Specific description of the mismatch]
...
Save to `ad-lp-audit-[YYYY-MM-DD].md` in the current working directory (or a user-specified path).

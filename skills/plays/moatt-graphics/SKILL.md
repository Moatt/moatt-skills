---
name: moatt-graphics
description: Portable visual skill pack for the Agent Skills ecosystem (Claude Code, Claude Desktop, Claude Cowork, Claude Design, Moatt, Cursor, Codex). Discovers community-published styles + formats via the moatt CLI, runs an extract-style workflow on reference images, and exports rendered PNGs via Playwright.
---
...
It loads in any host that reads `SKILL.md` — Claude Code agents calling it for automated render, Moatt pipelines wiring it into content generation, Cursor/Codex projects picking it up via `.cursor/rules/` or `~/.codex/skills/`. Each style ships a slim DESIGN.md spec via `npx moatt styles get <slug>`, which can also be uploaded directly into Claude Design as a design-system scaffold.
...
```
/moatt-graphics --style <slug> --format <format> [--brief "..."] [--ref <image-path>]
```

- `--style <slug>` — any community-published style slug. Discover with `npx moatt styles list` / `search`. Required for style-selected generation. Omit to let the user pick interactively.
- `--format <format>` — any community-published format slug (e.g., `carousel`, `story`, `infographic`, `slides`, `poster`, `chart`, `tweet`, plus any community additions). Discover with `npx moatt formats list`. Required for format-selected generation.
- `--brief "..."` — the topic / content description. Replaces the Content Discovery phase.
- `--ref <image-path>` — if present, extract an ad-hoc style from the reference image for this single render instead of using a preset. When `--ref` is provided, `--style` is ignored. **For creating a persistent published style that other agents can discover, use the dedicated `/moatt-graphics-create-style` skill instead.**
...
/moatt-graphics --style matt-gray --format carousel --brief "How founders find their first 100 customers in 2026"

# Reference-driven — extract a one-off style from an image and build with it
/moatt-graphics --ref ~/Desktop/mood.png --format poster --brief "Summer studio open house, August 14"
...
To **create a persistent published style** (slim spec + hero render + a few additional examples, pushed to the central library so other agents can discover it), use the dedicated `/moatt-graphics-create-style` skill instead.
...
Styles and formats live in the central Moatt library, not in this skill pack. **Always list or search before assuming a slug exists** — the catalog is community-driven and changes over time.

```bash
# List or search styles
npx moatt styles list
npx moatt styles search "warm editorial"
npx moatt styles list --mood "Organic & Warm" --tag warm
...
npx moatt styles get <slug>

# Same shape for formats
npx moatt formats list
npx moatt formats search "vertical"
npx moatt formats get <slug>
```
...
When the user creates a new aesthetic via `extract-style.md` (or via the dedicated `/moatt-graphics-create-style` skill) and is happy with it, publish it to the community library so other agents and users can discover it:

```bash
cd <directory-with-rendered-examples>
npx moatt styles publish
```

The CLI expects a `moatt-style.json` manifest in the working directory plus the referenced PNG files. See **§13 Manifest format** for the exact shape.
...
When the existing community-published formats don't fit the user's canvas (e.g., LinkedIn banner 1584×396, story cover 1080×1920, event flyer 8.5×11in), use the sibling `/moatt-graphics-create-format` skill to produce a format spec and at least one rendered example, then publish:

```bash
cd <directory-with-format-bundle>
npx moatt formats publish
```

Format publishing **requires at least one rendered example** — the backend rejects an empty `examples` array. See **§13 Manifest format** for the exact shape of `moatt-format.json`.
...
`SKILL.md` (this file) auto-loads on most Agent Skills hosts once the pack is installed at the right path. Claude Design is the one host that does not auto-load skill packs — instead, it consumes individual style files (DESIGN.md) one at a time via manual upload, fetched on demand via `npx moatt styles get <slug>`.
...
Ask the user what they want to create. Run `npx moatt formats list` to see the current catalog. The seven baseline formats are:
...
> Browse the style catalog at **https://skills.moatt.com/styles** to see every published style with rendered examples.

Then run `npx moatt styles list` (or `search "..."` / `list --mood "..." --tag ...`) to surface relevant options inline. Present the candidates in groups by mood with their taglines so the user can pick.

The user may also say: **"I have a reference image"** — in that case, read `extract-style.md` (in this skill pack) and follow its workflow to derive a custom style from the provided image. That workflow ends in a publishable bundle (`moatt-style.json` + rendered PNGs) the user can publish via `npx moatt styles publish`. After publishing, continue the workflow from Step 3 onward using the new slug.
...
Default location: `./moatt-graphics-exports/`
...
**Map the format's `width`/`height` (from `npx moatt formats get <slug>`) to a canvas slug using this table:**
...
- **"Surprise me"** — Pick the carousel format and a random style returned by `npx moatt styles list`. Ask the user only for content/topic, then generate everything else automatically.
...
### 17.1 Style manifest (`moatt-style.json`)
...
- `examples[].format`: slug of an existing graphics format (any from `npx moatt formats list`)
- `examples[].file`: relative path to the PNG in the working directory

### 17.2 Format manifest (`moatt-format.json`)
...
This skill pack ships only the local helpers needed to render and source images — styles, formats, and example renders all come from the central library via `npx moatt`.
...
| File | Description |
|------|-------------|
| `extract-style.md` | Derive a custom style from a reference image and produce a publishable bundle (`moatt-style.json` + rendered PNGs). |

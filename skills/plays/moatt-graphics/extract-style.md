...
Extract the visual design language from a reference image and produce a publishable style bundle — a `moatt-style.json` manifest plus rendered example PNGs — that the user can push to the central Moatt library via `npx moatt styles publish`.
...
- `moatt-style.json` — the manifest (the slim-format spec lives in its `designMd` field)
- `<format>.png` — at least one rendered example (the hero); 2–3 recommended

The user then runs `npx moatt styles publish` from that directory to push the style to the catalog, where it becomes immediately discoverable by other agents via `npx moatt styles list/search/get`.
...
Generate a slim style spec — the markdown that will live in the manifest's `designMd` field. This is the same structure used by every published Moatt style. Do NOT generate the verbose 9-section DESIGN.md format.

If you need a structural reference, fetch one of the established slim specs from the catalog:

```bash
npx moatt styles get brutalist
npx moatt styles get matt-gray
```
...
**Important:** Study the component snippets in `npx moatt styles get brutalist` and `npx moatt styles get matt-gray` for the right level of detail. Each snippet should be a complete, copy-pasteable HTML block with all styles inline.
...
**Collision check:** run `npx moatt styles get <slug>` — if the catalog returns a hit, the slug is taken. Suggest an alternative.
...
1. Fetch the format spec: `npx moatt formats get <format>`.
...
Write `<working-dir>/moatt-style.json` matching the shape documented in `SKILL.md` §17.1. Required fields:
...
- `examples`: minimum 1, exactly one with `isHero: true`. `format` must be a slug returned by `npx moatt formats list`.
...
The CLI reads `moatt-style.json`, uploads the referenced PNGs to S3, and registers the style in the catalog. On a 409 slug collision, the CLI re-submits with the suggested slug.

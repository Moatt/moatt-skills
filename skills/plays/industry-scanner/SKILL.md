...
description: >
  Daily industry intelligence scanner. Sweeps web, social media, news, blogs, and
  communities for industry-relevant events, trends, and signals. Produces a
  comprehensive intelligence briefing plus strategic GTM opportunity ideas.
  Orchestrates existing scraping skills — does not reimplement data collection.
---

# Industry Scanner

Daily deep-research agent that sweeps the internet for everything relevant to a client's industry, then synthesizes strategic GTM opportunities from what surfaces.
...
- **Client name** — picks which config and context files to load
- **Lookback period** (optional) — `1` for daily (default), `7` for weekly deep scan
- **Focus area** (optional) — restrict the scan to specific categories (e.g., "competitors only", "events only")
...
1. Read `clients/<client>/config/industry-scanner.json` — keywords, sources, competitors, and URLs to scan
2. Read `clients/<client>/context.md` — needed for ICP, value props, and positioning when generating strategies
3. Set the lookback: `1` day for daily scans, `7` for weekly, or whatever the user specifies
4. Note today's date for the output filename

If no client config exists, ask the user for the key inputs and offer to scaffold one from the example at `skills/industry-scanner/config/example-config.json`.

### Phase 2: Data Collection

Run these data sources in parallel where possible. Skip any source that isn't configured. For each one, use the existing skill's CLI or tool as documented.

**IMPORTANT:** Fire as many of these bash commands in parallel as you can to keep total scan time low. Sources are independent of each other.
...
Also search each competitor name directly to catch any recent news.
...
See `skills/blog-feed-monitor/SKILL.md` for full CLI reference.
...
Also run a separate `--sort top --time week` to catch high-engagement posts.

See `skills/reddit-post-finder/SKILL.md` for full CLI reference.
...
See `skills/twitter-mention-tracker/SKILL.md` for full CLI reference.
...
See `skills/linkedin-post-research/SKILL.md` for the full Apify workflow.
...
Run once per configured `hn_queries` entry. See `skills/hacker-news-scraper/SKILL.md` for full CLI reference.
...
See `skills/accounting-news-monitor/SKILL.md` for full CLI reference.
...
See `skills/newsletter-monitor/SKILL.md` for full CLI reference.
...
See `skills/review-site-scraper/SKILL.md` for full CLI reference.

### Phase 3: Consolidate & Categorize

Once all collection completes, consolidate:

1. **Deduplicate** — items appearing across multiple sources (e.g., a news story on both a blog and Reddit). Keep the richest version but note the multi-source appearance (higher signal).
...
4. **Filter out noise** — Drop items rated Low relevance unless they're genuinely noteworthy. The aim is signal, not volume.

### Phase 4: Generate Strategic Opportunities

Review the consolidated intelligence and identify items (or clusters of related items) that represent real GTM opportunities.

**CRITICAL: Do NOT force-fit a strategy for every item.** Many items are just "good to know" — that's fine, they go in the briefing. Only generate strategy ideas where there's a real, actionable opportunity that could meaningfully move growth.

For each genuine opportunity, produce:

| Field | Description |
|-------|-------------|
| **Trigger** | What happened — the intelligence item(s) that sparked the idea |
...
Use these as inspiration, not a checklist. Match the pattern to the trigger:
...
Save the report to the current working directory as `industry-scan-<YYYY-MM-DD>.md` (or the user-specified path) using this structure:
...
- **Daily vs Weekly:** Daily scans (`--lookback 1`) are fast but may miss slower-moving stories. Run a weekly deep scan (`--lookback 7`) every Monday for comprehensive coverage.
- **Noisy sources:** When a source consistently returns irrelevant results, tune the config keywords instead of dropping the source.
- **Multi-source signals:** Items that appear across multiple sources (e.g., Reddit and Twitter) are higher-signal. Flag those in the briefing.
...
- **Follow up:** When an opportunity references a downstream skill (e.g., `company-contact-finder`), the user can chain straight into that skill to take action.
...
- `APIFY_API_TOKEN` — LinkedIn post search runs through the `linkedin-post-research` skill (Apify-based)

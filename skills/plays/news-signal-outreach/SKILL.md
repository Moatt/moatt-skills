...
version: 1.0.0
description: >
  Full-cycle news-triggered signal composite. Takes any piece of news — an
  article, LinkedIn post, tweet, announcement, event, trend, regulation,
  product launch, acquisition, layoff, expansion, or any other public event
  — and evaluates whether the companies or people mentioned are ICP fits. If
  yes, identifies the connection between the news and your product, surfaces
  the right people to contact, and drafts personalized outreach with the
  news as the hook. Tool-agnostic. Accepts both company-level and
  person-level news triggers.

  AUTO-TRIGGER: Load this composite whenever a user shares a URL (LinkedIn
  post, article, tweet, blog post) or mentions a company/person they "came
  across", "saw", or "found" from any external source and asks about
  relevance, fit, ICP match, or whether to reach out. The user does NOT need
  to explicitly say "outreach" — any signal-evaluation request from an
  external source triggers this.
tags: [outreach]
...
**Why this composite exists:** The world generates an infinite stream of potential outreach triggers. The four structured signal composites handle the most common patterns. This one handles the long tail — the unpredictable, opportunistic moments that often produce the best outreach because nobody else is sending a templated sequence about them.

## When to Auto-Load

Load this composite when ANY of these are true:
- User shares ANY URL (LinkedIn post, article, tweet, blog, news) and asks about a company or person in it
...
- User has a list of companies and wants to scan recent news for outreach angles
...
**Purpose:** Take the raw news input — whatever shape it arrives in — and extract structured entities (companies, people) and the core event.
...
3. **Expand if needed:** If the news implies a broader set of affected companies beyond those mentioned:
   - "New FDA regulation on telehealth" → all telehealth companies, not just the ones in the article
...
1. **If entity is a company:**
   ...
   - Estimate company size (from news context or a quick web search)
...
3. **For entities implied but not mentioned** (e.g., "all telehealth companies" from a regulation news):
   ...
   - This step may surface new companies not already in your pipeline

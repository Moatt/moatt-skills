...
description: >
  End-to-end website audit through ICP eyes. Builds synthetic personas (if they
  don't already exist), runs a structured scorecard review of the client's site,
  then runs a head-to-head competitive comparison against top competitors. Emits
  a single consolidated report with persona feedback, competitive positioning, and
  prioritized recommendations. The complete "how do our buyers actually experience
  our site vs the competition?" workflow.
tags: [research]
---
...
**Check whether personas already exist:**

```
clients/<client>/personas/personas.json
```

**If personas exist:** Load them, sanity-check that they're still current, and list them for the user. Skip to Phase 2.
...
4. **Cross-persona competitive summary:**
   ...
   - At-risk segments (personas who lean toward the competitor)
...
Merge everything into a single audit report. This is the deliverable.

**Synthesis work:**

1. **Combine scorecard + competitive findings** — scorecard shows absolute quality, head-to-head shows relative positioning. Both lenses matter.
...
3. **Rank recommendations by:**
   ...
   - How much it would shift conversion likelihood (depth)
...
Save to the current working directory as `icp-website-audit.md` (or the user-specified path).
...
If personas already exist, the workflow collapses to:
```
...
- **Start with 1-2 competitors, not 5.** Each one adds a full head-to-head evaluation pass. Start focused, add more if needed.
- **The at-risk segments are the most actionable finding.** If a persona consistently prefers a competitor, that's an ICP segment you're actively losing deals in. That should drive the urgency.
- **Reuse this quarterly.** Build personas once, then re-run the audit after site changes or quarterly to track improvement. Compare scores across dates.
- **Combine with the SEO content audit for the full picture.** ICP audit tells you "does the messaging work for buyers?" SEO audit tells you "are buyers finding you in the first place?" Together they cover the full funnel.
- **This makes a strong client deliverable.** The consolidated report is structured for sharing. Use `content-asset-creator` to make it into a polished HTML report.
- **Don't skip the competitive analysis.** The scorecard alone is useful, but the head-to-head is where the real insight lives. Buyers always compare — your audit should too.
...
Free — no paid APIs. Every step uses web search and web fetch.

...
version: 1.0.0
description: >
  Triages every inbound lead from a given period — demo requests, free trial
  signups, content downloads, webinar registrations, chatbot conversations.
  Classifies by urgency, qualifies against ICP, enriches with context, and
  produces a prioritized action queue with a recommended response for each
  lead. Tool-agnostic — works with any CRM, form tool, or lead source.
tags: [lead-generation]
---

# Inbound Lead Triage

Sweeps every inbound lead that landed in a chosen time window, sorts them by source urgency, runs an ICP fit check, layers on company/person context, and emits a prioritized action queue. Transforms a chaotic pile of form fills into a clear ranked list of who deserves a reply first.
...
On the first run, ask the user to configure their lead sources and preferences. Save to the current working directory or wherever the user prefers (e.g., `config/inbound-triage.json`).
...
- Lead sources to check (every configured source by default, or user specifies)

### Process
1. Pull leads from each configured source for the chosen period
...
3. Deduplicate by email — if the same person appears in multiple sources, merge into one record listing all source types (this is a stronger signal)
...
### Human Checkpoint
Present the collected lead counts by source. Ask: "These are the leads I found. Proceed with triage?"
...
Rank every lead into one of four urgency tiers based on source type and behavioral signals.
...
- Person has a title that maps to a buyer persona
...
- Are they already in our pipeline? (check your CRM — HubSpot, Salesforce, CSV)
...
Also produce a flat CSV with every lead and its triage data:
- All original fields
...
- **Daily AM** (minimum): Morning triage of all leads from the previous day/overnight

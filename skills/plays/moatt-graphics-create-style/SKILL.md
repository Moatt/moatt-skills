---
name: moatt-graphics-create-style
description: >
  End-to-end skill that turns a single reference image into a published
  Moatt style — analyzes the image, drafts the slim style spec, renders
  a hero example plus 2-3 additional formats via Playwright, writes the
  `moatt-style.json` manifest, and publishes via
  `npx moatt styles publish` so other agents can discover it. Mirrors
  moatt-graphics-create-format but for styles.
tags: [design, content]
---

# Create Moatt Graphics Style

Authors a new graphics style and publishes it to the central Moatt
library via `npx moatt styles publish`. The output is a working
directory with a `moatt-style.json` manifest plus a hero rendered
...
`/moatt-graphics --style <slug> --format <any>` call can render against it.

**Always check first:** run `npx moatt styles list` (or
`npx moatt styles search "warm editorial"`) to see whether a
community-published style already covers the look. If one fits, just use
it via the regular `moatt-graphics` flow.

## Prerequisites

- The `moatt-graphics` skill must be installed in the same workspace —
  this skill uses its `screenshot/screenshot.js` to render examples.
  Install via:
  ```bash
  npx moatt install --claude --with moatt-graphics
  ```
  (Swap `--claude` for `--cursor` or `--codex` as needed.) See the install
  page on the hub for the canonical command:
  https://skills.moatt.com/skills/moatt-graphics
- The screenshot tool's dependencies must be installed
  (`moatt-graphics/screenshot/node_modules/` must exist). If not:
  ```bash
  cd <path-to>/moatt-graphics/screenshot && npm install && npx playwright install chromium
  ```
- The user must be signed in to Moatt (`npx moatt login`) for
  `publish` to authenticate.

## Invocation

```
/moatt-graphics-create-style --ref <image-path> [--name <slug>] [--mood <mood-group>]
```
...
If the user says "I want a new style for moatt-graphics" without args, ask
...
```
<working-dir>/
  moatt-style.json   # manifest (see "Manifest format" below)
...
**Collision check:** run `npx moatt styles get <slug>` — if the
...
```bash
npx moatt styles get dot-grid-stat
```
...
```bash
node <path-to>/moatt-graphics/screenshot/screenshot.js \
  --format <format> \
  --input <path-to-html> \
...
### Step 5 — Write `moatt-style.json`

Match the shape documented in `moatt-graphics/SKILL.md` §17.1. Required:
...
- `examples[].format`: slug of an existing graphics format
  (any from `npx moatt formats list`).
...
The CLI reads `moatt-style.json`, validates it client-side, uploads
...
https://skills.moatt.com/styles/<slug>
```
...
- The exact command future agents will use to render with the new style:
  `/moatt-graphics --style <slug> --format <format> --brief "..."`.
...
- Fetch a canonical slim spec to mirror its shape:
  `npx moatt styles get dot-grid-stat`,
  `npx moatt styles get pillow-block`,
  `npx moatt styles get blush-annual`,
  `npx moatt styles get neon-dashboard`.
- `moatt-graphics/screenshot/screenshot.js` is the rendering pipeline; it
  sets format-specific viewport and capture mode.
- `npx moatt formats get <format>` returns each format's spec and
...
- **Don't publish without checking `npx moatt styles list` first** —

...
description: >
  Full GTM analysis of a target company. Researches what a company is
  currently doing across every go-to-market dimension — content/blog,
  founder LinkedIn activity, SEO/traffic, hiring signals, social/community
  presence, customer acquisition channels, podcast appearances, review sites,
  competitive positioning, and partnerships. Produces a structured report
  identifying what's working, what's missing, and where white space exists
  for new strategies. Run this before designing GTM strategies for a client
  or prospect.
---

# Company Current GTM Analysis

Deep-research agent skill that produces a thorough analysis of what a target company is currently doing across every go-to-market dimension. The output surfaces what's working, what's missing, and where white space exists — so you can design strategies that fill real gaps rather than duplicating what they already do.
...
| **Founder/exec LinkedIn URLs** | Recommended | User provides, or search the web for "[founder name] LinkedIn" |
| **Known competitors** | Optional | Improves the competitive positioning section |
| **Client context file** | Optional | `clients/<client>/context.md` — if it exists, use it for ICP, positioning, etc. |
...
1. If a client context file exists at `clients/<client>/context.md`, read it for:
   ...
   - Any prior research or intelligence
...
Fire as many of these research threads in parallel as possible. Each thread is independent.

#### 2A. Blog & Content Strategy

**Goal:** Understand the content strategy — topics, frequency, formats, target audience, gaps.

1. **Fetch the blog page:**
   - WebFetch `<website>/blog` — pull all visible post titles, dates, categories, authors
   - If no `/blog` exists, try `/resources`, `/insights`, `/news`, `/articles`
...
**Goal:** Understand founder thought-leadership presence, posting frequency, engagement, and topics.
...
2. **Analyze the posts:**
   ...
   - Whether they engage in comments (replying to commenters)
...
**Goal:** Understand how they win customers and what those customers think.

1. **Review sites:**
   - WebSearch: `<company> OR <product> review G2 Capterra <current year>`
   - WebFetch G2/Capterra pages when URLs are findable
   - Note: rating, review count, common praise, common complaints
   - Optional: use the `review-site-scraper` skill for detailed extraction
...
5. **Paid advertising signals:**
   - Check for Facebook Pixel or Google Ads tags on their site
...
Once all data collection completes, synthesize into a structured analysis.
...
**Date:** <YYYY-MM-DD>
**Purpose:** Understand what <Company> is currently doing across every GTM
dimension so we can identify white space for new strategies.
...
- **Parallelize aggressively.** Phases 2A through 2G are all independent — run them at the same time to minimize total research time. Use background agents or parallel tool calls wherever possible.
...
- **Grade relative to stage.** A 10-person seed startup doesn't need analyst coverage. A $30M Series A company does. Calibrate your grading to what's appropriate for their stage.
...
- **Check the existing client workspace first.** If `clients/<client>/` already has research, intelligence reports, or strategy docs, read them before re-researching. Build on existing knowledge.

## Dependencies

- `APIFY_API_TOKEN` env var — for LinkedIn profile post scraping
- `requests` (Python) — for the LinkedIn scraper script

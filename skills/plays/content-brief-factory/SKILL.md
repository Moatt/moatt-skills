...
description: >
  Generate detailed, differentiated content briefs at scale. Each brief bundles
  SERP analysis, competing-page breakdown, unique angles harvested from real customer
  language (reviews, Reddit), an internal linking plan, and SERP-feature targets.
  Batch mode produces 10-50 briefs per run. Beats the generic "keyword density"
  briefs that tools like Surfer or Clearscope hand back.
tags: [content]
---
...
Most content briefs are garbage. They say "mention these keywords 7 times" and call it a strategy. This skill produces briefs that include competitive analysis, unique angles harvested from what real customers say, SERP-feature targeting, and a differentiated outline — so your writer knows exactly what to create and why it'll rank.
...
Briefs improve significantly with real SERP data (what's actually ranking, what SERP features appear) instead of inferred analysis.

### Agent Prompt to User

> "I can put together strong content briefs using competitive research and customer language mining. For the sharpest results — especially accurate SERP analysis and keyword metrics — I'd recommend connecting a SERP data API."
>
> **Recommended: DataForSEO SERP** (pay-per-use, ~$0.01/search — ToS-clean, JSON-stable, no monthly subscription)
...
>
> **Other supported (fallback):** SerpAPI (`SERPAPI_KEY`), Serper.dev (`SERPER_API_KEY`), or ValueSERP (`VALUESERP_API_KEY`). Set the corresponding env var to enable. DataForSEO is preferred for ToS-cleanness, JSON stability, and lower per-query cost.
>
> "Want to use DataForSEO or a fallback, or should I proceed with baseline mode? Baseline uses web search to analyze competing pages — still produces great briefs, just with less precise SERP-feature and volume data."

### Mode Selection

- **Recommended mode (DataForSEO)** — Real SERP data per keyword: exact ranking pages, SERP features present (featured snippets, PAA, video carousels), search volume, keyword difficulty. Briefs include specific "win this SERP feature" targeting. Falls back to SerpAPI, Serper.dev, or ValueSERP if their env vars are set instead.
...
7. **Tool preference** — Enhanced mode with a SERP API, or baseline? (see Tool Enhancement above)

## Phase 1: Existing Coverage Check

Run `site-content-catalog` against your site:
...
- What content gaps exist? → Confirms the keyword is worth targeting
...
**Recommended mode (DataForSEO). Falls back to SerpAPI, Serper, or ValueSERP if their env vars are set.**
...
- Identify obvious SERP features from the result format
- Use `seo-domain-analyzer` for volume/difficulty estimates

### 2B: Competing Page Analysis

For the top 3-5 ranking pages per keyword, fetch with `fetch_webpage`:
...
### Meta Description
[155 characters, includes the keyword, carries a clear value prop]
...
| DataForSEO SERP (recommended, per keyword) | ~$0.01 per keyword |
| Fallback tools (SerpAPI, Serper.dev, ValueSERP) | ~$0.001-0.05 per keyword (if using instead of DFS) |
...
- **Recommended:** DataForSEO via Moatt (`MOATT_API_KEY`) — pay-per-use, ~$0.01 per query metered on your Moatt account, no monthly subscription.
- **Other supported (fallback):** SerpAPI (`SERPAPI_KEY`), Serper.dev (`SERPER_API_KEY`), or ValueSERP (`VALUESERP_API_KEY`). Set the corresponding env var to enable. DFS is preferred for ToS-cleanness, JSON stability, and lower per-query cost.

## Scheduling

For ongoing content operations, run weekly:
- Input: keywords from `seo-opportunity-finder` or the content calendar

...
description: >
  Watch competitor pricing pages via live scrape and Web Archive snapshots. Track plan
  changes, tier restructuring, new pricing models, and feature gating shifts. Produces a
  pricing comparison matrix and flags whenever a competitor changes packaging. Run when
  product marketing needs to stay current on competitive pricing, or before changing your
  own.
...
Watch competitor pricing pages over time. Catch plan changes, feature-gating shifts, pricing-model swaps, and new tiers. Output is a living pricing comparison matrix plus alerts when anything moves.

**Core principle:** Pricing is the most under-monitored competitive signal. Most teams only check competitor pricing right before they change their own. This skill makes it continuous.
...
5. **First run or recurring?**
   ...
   - **Recurring:** Compare against the last snapshot
...
- **Add-ons** — What costs extra beyond the base plans?
...
Look up past versions of their pricing page:
...
Pull the last 2-3 snapshots to detect:
...
Build a normalized comparison across competitors:
...
For the ICP's typical use case, calculate the effective cost:
...
This shows the real competitive pricing position, not just list price.
...
Compare the current snapshot against the previous:
...
Features that matter most to the ICP — where do they sit?
...
- **Opportunity:** [Gap in the market — e.g., "no one offers usage-based in this category"]
